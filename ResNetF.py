...
# https://hypnettorch.readthedocs.io/en/latest/_modules/hypnettorch/mnets/resnet.html#ResNet#
...
hyper_shapes_distilled` and the current statistics will be returned by the
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(ResNetF, self).__init__(num_classes, verbose)
        ...
        self._context_mod_post_activation = kwargs[
        ...
        if len(num_feature_maps) != 4:
            raise ValueError(
                'Option "num_feature_maps" must be a list of 4 ' + "integers."
            )
        ...
        for i in range(1, 4):
            ...
            if num_feature_maps[i] < num_feature_maps[i - 1]:
                raise ValueError(
                    "We currently require the number of "
        ...
        self._internal_params = (
            None
            if no_weights and self._context_mod_no_weights
        ...
        self._context_mod_layers = (
            nn.ModuleList() if self._use_context_mod else None
        )
                    ...
                    self._param_shapes_meta.append(
                        {
                            ...
                            "index": -1
                            if no_weights
                            else len(self._internal_params) + ii,
                ...
                self._param_shapes_meta.append(
                    {
                        ...
                        "index": -1
                        if no_weights
                        else len(self._internal_params) + ii + 1,
                        "layer": -1,  # TODO implement
                    }
                )
        ...
        num_weights = (
            ...
            * (
            ...
            + fs[-1] * num_classes
            + (num_classes if self.has_bias else 0)
        )

        cm_num_weights = (
            2 * MainNetInterface.shapes_to_num_weights(cm_shapes)
            if self._use_context_mod
            else 0
        )
        ...
        for i in range(1, len(self._filter_sizes)):
            ...
            for _ in range(2 * n):
                self._layer_weight_tensors.append(
                    nn.Parameter(
                        torch.Tensor(
                            out_filters, in_filters, *self._kernel_size
                        ),
                        requires_grad=True,
                    )
                )
        ...
        if init_weights is not None:
            ...
            for i in range(len(init_weights)):
                assert np.all(
                    np.equal(list(init_weights[i].shape), self.weights[i].shape)
                )
    ...
    def forward(self, x, weights=None, distilled_params=None, condition=None):
        ...
        Returns:
            (torch.Tensor): h: The output of the network.
            (torch.Tensor): features: Extracted features after average pooling,
                            before the last output layer
        """
        if (
        ...
        h = F.linear(features, w_weights[layer_ind], bias=b_weights[layer_ind])
